...
const PeticaoDocumentoEditar = ({
  ...
  // Template padrão caso não tenha conteúdo inicial
...
<div style="line-height: 1.5; margin-left: 40px; margin-right:40px;">
  ...
  // Carrega o conteúdo inicial ou template padrão
  useEffect(() => {
    if (editorRef.current && !conteudoCarregado) {
      let conteudoParaCarregar = conteudoInicial;

      // Se não tem conteúdo inicial, usa o template padrão
      ...
      // Notifica o componente pai sobre o conteúdo inicial
      if (onConteudoChange) {
...
<body>
  ...
  const toggleFormat = (format) => {
    ...
    // Notifica mudança após formatação
    handleInput();
  };
  ...
  const setAlign = (align) => {
    ...
    // Notifica mudança após alinhamento
    handleInput();
  };
  ...
  const changeFontFamily = (font) => {
    ...
    // Notifica mudança após alteração de fonte
    handleInput();
  };
